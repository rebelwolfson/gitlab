...
export default async (pluginConfig, context) => {
  ...
  const { gitlabToken, gitlabUrl, gitlabApiUrl, assets, milestones, proxy, rejectUnauthorized } = resolveConfig(pluginConfig, context);
  const assetsList = [];
      ...
      allAssets.map(async (asset) => {
        ...
        if (_url) {
          ...
          if (target === "generic_package") {
            ...
            try {
              response = await got.put(uploadEndpoint, { ...apiOptions, ...proxy, body: readFileSync(file), https: { rejectUnauthorized }, }).json();
            } catch (error) {
            ...
            try {
              ...
              response = await got.post(uploadEndpoint, { ...apiOptions, ...proxy, body: form, https: { rejectUnauthorized }, }).json();
            } catch (error) {
