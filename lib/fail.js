...
export default async (pluginConfig, context) => {
  ...
  const { gitlabToken, gitlabUrl, gitlabApiUrl, failComment, failTitle, labels, assignee, rejectUnauthorized } = resolveConfig(
    pluginConfig,
    context
  );
  ...
  if (failComment === false || failTitle === false) {
    ...
    const openFailTitleIssues = await got(openFailTitleIssueEndpoint, { ...apiOptions, https: { rejectUnauthorized }, }).json();
